...
class CAdmin
{
	...
	{	
		...
		if ($admins) {
			...
			$params = array('user'=>$_SESSION['admin'], 'content' => 'admin_manage.html.twig', 'admins' => $admins);
			$template->display($params);
		}
		else {
	...
	{
		$template = $twig->loadTemplate('admin.html.twig');
		$params = array('user'=>$_SESSION['admin'], 'content' => 'registration.html.twig');
		$template->display($params);
	}
	...
	public function resetpass($pdo, $twig)
	{
		...
		if (isset($_GET['id'])) {
			...
			$template = $twig->loadTemplate('admin.html.twig');
			$params = array('user'=>$_SESSION['admin'], 'content' => 'admin_reset_password.html.twig', 'admin' => $admin);
			$template->display($params);
		}
	}
